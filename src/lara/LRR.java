...
public class LRR extends RatingRegression {
	...
	// if we want to load previously trained models
	...
	protected double infer_alpha(Vector4Review vct) throws ExceptionWithIflag{
		...
		if (m_diag_alpha==null) {
			m_diag_alpha = new double[n];
			m_g_alpha = new double[n];
			m_alpha_cache = new double[n];
		} else 
...
//		model.SaveModel("Data/Model/model_hotel.dat");
		...
		model.LoadVectors("Data/Vectors/Vector_CHI_fiveHotel.dat");
		model.SavePrediction("Data/Results/prediction_fiveHotel.dat");
	}
}
