...
public class RatingRegression {
	...
	protected int m_v, m_k;//number of features (i.e., vocabulary size), number of aspects
...
//		m_rand = new Random(System.currentTimeMillis());//pseudo random number
	}
	...
	public int LoadVectors(String filename, int size){
		try {
			...
			while((tmpTxt=reader.readLine())!=null){
				pos = tmpTxt.indexOf('\t');
				if (isTrain = (m_rand.nextDouble()<0.75))//train/test splitting ratio, default 4-fold CV
					m_trainSize++;
				else
					m_testSize++;
				...
				for(int i=0; i<vct.getAspectSize(); i++){
					...
					vct.setAspect(i, tmpTxt.split("\\s+"));//white space as delimiter
